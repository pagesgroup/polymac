...
Web.on('loaded', (event) => Abis.config({serviceRoot,socketRoot}).init().then(async (abis) => {
              ...
              $('table').class('grid').style('width:100%;').append(
                ...
                $('tbody').append(
                  taken.filter(item => item.projectnr == project.nr).map(item => $('tr').append(
                    ...
                    $('td').style('text-align:right;').text(num(item.Calc,0)),
                  )),
                  $('tr').append(
                    $('th').text('Totaal').style('font-weight:bold;'),
                    $('td').style('text-align:right;').text(num(taken.filter(item => item.projectnr == project.nr).map(item => item.Calc).reduce((t,v) => t+v, 0),0)),
                  ),
                ),
              ),
  ...
  loadExcelData('http://10.10.60.31/engineering/Projects/Planning/Planning Systems.xlsx').then(async data => {
    ...
    // const weburen = await Aim.fetch('http://10.10.60.31/api/exact_weburen').get();
    const weburen = await Aim.fetch('http://10.10.60.31/api/weburen_totaal').body({nrs:Item.task2.map(task => task.ordernr).unique().filter(Boolean).join(',')}).post();
    const {weburen_totaal} = body;
    // console.log(weburen_totaal,weburen_totaal.filter(u=>u.ordernr==20235053));
    Item.task2.forEach(task => {
      task.budget = task.budget || 0;
      task.tegaan = task.tegaan || 0;
      task.besteed = task.besteed || 0;
    });
    weburen_totaal.forEach(uren => {
      const task = Item.task2.find(task =>
    ...
    Item.task2.filter(task => task.nietinplanning).forEach(task => task.notes.push(`Taak komt niet voor in planning! Geen budget! Uren geboekt ${num(task.besteed,1)}.`));
    Item.task2.filter(task => task.resources && !task.budget && task.tegaan).forEach(task => task.notes.push(`Geen budget ingevoerd! Wel nog ${task.tegaan} uren te besteden door ${task.resources}`));
    Item.task2.filter(task => task.resources && task.tegaan == 0.1).forEach(task => task.notes.push(`Uren te gaan onbekend. Graag uren opgeven voor deze taak voor ${task.resources}`));

    // console.log(Item.task2.filter(task => task.nietinplanning).map(task => [task.ordernr,task.orderdeel,task.deel,task.activiteit].join('\t')).join('\n'));

    const resources = Item.task2.filter(task => task.resources).map(task => task.resources.split(';')).flat().unique().sort().map(fullname => Object({fullname}));
    // console.log(Item.task2.filter(task => task.ordernr == 20225109));
    ...
    // project.filter(project => !project.leadengineer).forEach(project => project.notes.push('Wie is binnen Elma hoofdverantwoordelijk voor de engineering ofwel leadengineer?'));
    ...
    const maxuren = 40;
    const planweken = 40;
    const taken = Item.task2;//data.task.filter(item => item.Taak);
    ...
    taken.forEach(item => item.Besteed = 0);
    taken.forEach(item => item.projectnr = String(item.Nr||'').trim().split('.').shift());
    taken.forEach(item => item.projectdeel = String(item.Nr||'').trim().split('.').pop());
    weburen.forEach(row => {
      row.tasks = taken.filter(taak => taak.Nr == row.project && taak.Wie == row.fullname && taak.Deel == row.deel);
      row.tasks.forEach(taak => taak.Besteed += Number(row.totaal));
    })
    const resources = taken.map(item => item.Wie).unique().map(name => Object({name,weken:[]}));
    const weken = [];
    taken.forEach(item => item.weken = []);
    var date = new Date();
    ...
    taken.forEach(item => {
      ...
      var teGaan = item.TeGaan = item.TeGaan || 0;

      item.person = Item.person.find(person => person.id == item.Wie) || {};
      item.rate = item.person.rate || 80;
      item.cost = item.person.cost || 60;

      item.Calc = item.Calc || 0;
      item.Totaal = (item.Besteed||0) + teGaan;
      item.Open = (item.Calc||0) - (item.Besteed||0);
      item.Verwacht = (item.Besteed||0) + teGaan;
      item.Over = item.Open - teGaan;
      item.prognose = item.Calc * item.rate - (item.Besteed + item.TeGaan) * item.cost

      if (item.Start && item.Deadline) {
        const startDate = excelDateToJSDate(item.Start);
        const endDate = excelDateToJSDate(item.Deadline);
        var days = getWorkDays(startDate,endDate);
        const dayHours = teGaan / days;
        // console.log(startDate.toLocaleDateString(), endDate.toLocaleDateString(), days, dayHours);
        for (let date = new Date(startDate); date <= endDate;date.addDays(1)) {
          if ([1,2,3,4,5].includes(date.getDay())) {
            const i = weekindex(date);
            item.weken[i] += dayHours;
            resource.weken[i] += dayHours;
          }
        }
      } else if (item.Start) {
        const startWeek = weekyear(excelDateToJSDate(item.Start)) - currentWeek;
        ...
        for (let date = excelDateToJSDate(item.Deadline); teGaan; date.addDays(-1)) {
          if ([1,2,3,4,5].includes(date.getDay())) {
            const i = weekindex(date);
            const beschikbaar = Math.max(0,maxuren-resource.weken[i]);
            let uren = Math.min(beschikbaar,teGaan);
            ...
            $('table').class('grid').style('font-size:8pt;white-space:nowrap;border-collapse:collapse;').append(
              ...
              $('tbody').append(
                items.map(item => $('tr').append(
                  ...
                  $('td').text(item.Deadline ? excelDateToJSDate(item.Deadline).toLocaleDateString() : '').style('text-align:right;'),
                  $('td').text(item.Wie),

                  $('td').text(num(item.Calc||0,0)).style('text-align:right;'),
      ...
      return $('table').class('grid nowrap').append(
        $('thead').append(
          $('tr').append(
            ...
            $('td').text('Budget'),
            $('td').text('Besteed'),
            $('td').text('Te gaan').style('white-space:nowrap;'),
            $('td').text('Resultaat'),
            $('td').text('Voortgang'),
            $('td').text('Status'),
            $('td').text('Resources'),
        ...
        $('tbody').append(
          items.flat(9).sort(sorttasks).map(item => $('tr').append(
            $('td').text([item.ordernr,item.orderdeel].filter(Boolean).join('.')),
            $('td').text([item.klant,item.projectnaam].filter(Boolean).join(' > ')),
            $('td').text(item.activiteit),
            $('td').text(item.deel,item.omschrijving),
            $('td').text(item.begin ? excelDateToJSDate(item.begin).toLocaleDateString('nl-NL', {year: 'numeric', month: '2-digit', day: '2-digit'}) : '').style('text-align:right;'),
            $('td').text(item.eind ? excelDateToJSDate(item.eind).toLocaleDateString('nl-NL', {year: 'numeric', month: '2-digit', day: '2-digit'}) : '').style('text-align:right;'),
    ...
    Web.treeview.append({
      Planning: {
        children: {
                    ...
                    $('div').class('col').append(
                      $('details').append(
                        ...
                        projecten.map(project => $('details').append(
                          ...
                          tasktable(productionorder.filter(order => order.projectnr == project.projectnr).map(order => Item.task2.filter(uren => uren.ordernr == order.ordernr))),
                        )),
                      ),
                      $('details').append(
                        ...
                        resources.map(resource => $('details').append(
                          ...
                          tasktable(Item.task2.filter(task => task.tegaan && (task.resources||'').split(';').includes(resource.fullname))),
                        )),
                      ),
                      $('details').append(
                              ...
                              $('ol').style('list-style-type:decimal;').append(
                                projecten.filter(project => project.projectmanager === projectmanager && project.projectnr && project.notes.length).map(project => $('li').text('Project',project.projectnr).append(
                                  $('ol').style('list-style-type:decimal;').append(
                                    ...
                                    Item.productionorder.filter(order => order.projectnr === project.projectnr && Item.task2.filter(task => task.ordernr == order.ordernr && task.notes.length).length).map(order => $('li').text('Order',order.ordernr).append(
                                      $('ol').style('list-style-type:decimal;').append(
                                        Item.task2.filter(task => task.ordernr == order.ordernr && task.notes.length)
                                        .sort(sorttasks)
          ...
          //               ),
          //               $('td').style('text-align:right;').append(
          //                 $('div').text(num(taken.filter(item => item.projectnr == project.nr).map(item => item.Calc).reduce((t,v) => t+v, 0),0)),
