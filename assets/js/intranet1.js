...
Web.on('loaded', (event) => Abis.config({serviceRoot,socketRoot}).init().then(async (abis) => {
  ...
  // var keys = Object.keys(config.definitions).filter(schemaName => config[schemaName]);
  // console.log(keys);
  // var keys = ['faq'];

  Object.keys(config.definitions)
  .filter(schemaName => config[schemaName])
  .forEach(schemaName => {
    config.definitions[schemaName].prototype = {select};
    config[schemaName].forEach((item,id) => new Item({schemaName,id:schemaName+id},item))
  });

  console.log(Array.from(Item.items.values()));
  Web.search();


  // Web.search1 = function(searchText){
  //   const url = new URL(document.location);
  //   url.searchParams.set('search', searchText);
  //   window.history.replaceState('','',url);
  //   $('input.search').value(searchText);
  //   const searchArray = searchText.split(' ');
  //   const rows = Array.from(Item.items.values()).filter(row => searchArray.every(word => row.headers.join(' ').match(new RegExp(word,'i'))));
  //   console.log(rows);
  //   // rows.forEach(row => {
  //   //   if (!row.headers) {
  //   //     row.headers = [
  ...
  const searchData = [].concat(
    ...
    //
    // Object.values(config.afas).map(item => Object.assign(item, {header: [item.title].filter(Boolean).join(' > ')})),

    Object.values(config.afas).map(item => Item.get(item)),
  ...
  loadExcelData('http://10.10.60.31/engineering/Projects/2019/20195019%20Hydac%20Afsluitdijk/02-Project%20Documenten/20195019 Control projectvoortgangs document.xlsx').then(data => {
    console.log(1,data);
    return;
  }).finally(search);

  loadExcelData('http://10.10.60.31/engineering/Projects/2019/20195019%20Hydac%20Afsluitdijk/02-Project%20Documenten/Planning/20195019 ASD ELMA Milestones.xlsx').then(data => {
    console.log(2, data);
    return;
  }).finally(search);
  ...
  loadExcelData('http://10.10.60.31/engineering/Projects/Planning/Planning Systems.xlsx').then(data => {
    ...
    taken.forEach(item => {
      ...
      item.Totaal = (item.Besteed||0) + (item.TeGaan||0);
    ...
    function planning() {
      const rows = [];
      return $('div').class('col').style('width:0;').append(
        ...
        $('table').class('grid').style('font-size:8pt;white-space:nowrap;border-collapse:collapse;').append(
          ...
          $('tbody').append(
            ...
            ).style(item.Totaal > item.Calc ? 'color:red;' : '')),
          ),
        ),
        ),
      )
    ...
    $('button').parent('nav>.mw').text('Uren').on('click', event => {
      console.log('UREN');
    ...
    $('button').parent('nav>.mw').text('Project').on('click', event => {
      console.log('UREN');
      const name = window.localStorage.getItem('username');
      $('.listview').clear().append(
        $('div').append(
    ...
    $('button').parent('nav>.mw').text('Project R').on('click', event => {
      console.log('UREN');
      const name = window.localStorage.getItem('username');
      $('div').append(
